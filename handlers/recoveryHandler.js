...
class RecoveryHandler {
  ...
  initializeEventHandlers() {
    ...
    this.webSocketService.on('service.complete', this.handleServiceComplete.bind(this));

    // New: Realtime chat
    this.webSocketService.on('recovery.message.send', this.handleRecoveryMessageSend.bind(this));
    this.webSocketService.on('recovery.messages.get', this.handleRecoveryMessagesGet.bind(this));
    if (typeof this.handleRecoveryMessageRead === 'function') {
      this.webSocketService.on('recovery.message.read', this.handleRecoveryMessageRead.bind(this));
    }
    if (typeof this.handleRecoveryTyping === 'function') {
      this.webSocketService.on('recovery.typing', this.handleRecoveryTyping.bind(this));
    }

    // New: Price negotiation
    this.webSocketService.on('price.propose', this.handlePricePropose.bind(this));
    this.webSocketService.on('price.accept', this.handlePriceAccept.bind(this));
    this.webSocketService.on('price.reject', this.handlePriceReject.bind(this));

    // New: Discovery filters (pink/safety)
    this.webSocketService.on('discovery.filters.set', this.handleDiscoveryFiltersSet.bind(this));

    // New: Favourites & direct dispatch
    this.webSocketService.on('booking.toFavourite', this.handleBookingToFavourite.bind(this));
    this.webSocketService.on('favourites.add', this.handleFavouritesAdd.bind(this));
    this.webSocketService.on('favourites.list', this.handleFavouritesList.bind(this));
    this.webSocketService.on('favourites.remove', this.handleFavouritesRemove.bind(this));

    // New: Multi-stop rules
    this.webSocketService.on('multiStop.set', this.handleMultiStopSet.bind(this));
    this.webSocketService.on('multiStop.rules', this.handleMultiStopRules.bind(this));

    // New: Waiting/overtime consent
    this.webSocketService.on('service.waiting.consent', this.handleWaitingConsent.bind(this));

    // New: Presence signals
    this.webSocketService.on('presence.enroute', this.handlePresenceEnroute.bind(this));
    if (typeof this.handlePresenceStatus === 'function') {
      this.webSocketService.on('presence.status', this.handlePresenceStatus.bind(this));
    }
    // Aliases for presence
    this.webSocketService.on('presence.im_coming', this.handlePresenceEnroute.bind(this));
    this.webSocketService.on('presence.im_arrived', this.handleDriverArrival.bind(this));

    // New: Ratings, tips, billing
    this.webSocketService.on('rating.submit', this.handleRatingSubmit.bind(this));
    this.webSocketService.on('tip.add', this.handleTipAdd.bind(this));
    this.webSocketService.on('billing.get', this.handleBillingGet.bind(this));

    // New: Driver cancel with rebroadcast
    if (typeof this.handleDriverCancel === 'function') {
      this.webSocketService.on('driver.cancel', this.handleDriverCancel.bind(this));
    }

    // New: Fraud check hook
    if (typeof this.handleFraudCheck === 'function') {
      this.webSocketService.on('fraud.check', this.handleFraudCheck.bind(this));
    }

    // New: Services catalog for info buttons/images
    if (typeof this.handleServicesCatalogGet === 'function') {
      this.webSocketService.on('services.catalog.get', this.handleServicesCatalogGet.bind(this));
    }
  ...
  async handleRecoveryRequest(ws, message) {
    ...
    try {
      ...
      const creatorId = ws?.user?.id || ws?.user?._id?.toString?.() || data.customerId || data.userId || null;

      // Normalize incoming coordinates
      const pLat = data.pickupLocation.coordinates?.lat ?? data.pickupLocation.coordinates?.latitude;
      const pLng = data.pickupLocation.coordinates?.lng ?? data.pickupLocation.coordinates?.longitude;
      const dLat = (data.destinationLocation || data.dropoffLocation)?.coordinates?.lat ?? (data.destinationLocation || data.dropoffLocation)?.coordinates?.latitude;
      const dLng = (data.destinationLocation || data.dropoffLocation)?.coordinates?.lng ?? (data.destinationLocation || data.dropoffLocation)?.coordinates?.longitude;
      ...
      const dropoffGeo = (dLat != null && dLng != null) ? {
        type: 'Point',
        coordinates: [Number(dLng) || 0, Number(dLat) || 0],
        address: (data.destinationLocation || data.dropoffLocation)?.address || 'Unknown',
        zone: (data.destinationLocation || data.dropoffLocation)?.zone || 'general'
      } : {
        type: 'Point',
      ...
      const selectedRadiusKm = Math.max(1, Math.min(50, Number(data?.searchRadius) || (bookingDoc?.driverFilters?.searchRadius ?? 10)));

      // Map service type/category to schema enums
      const mapCategory = (t) => {
        const v = String(t || '').toLowerCase().trim();
        if (['towing', 'flatbed', 'wheel_lift', 'wheel-lift'].includes(v)) return 'towing services';
        if (['winching', 'on-road winching', 'off-road winching'].includes(v)) return 'winching services';
        if (['roadside', 'roadside_assistance', 'roadside assistance', 'battery', 'fuel'].includes(v)) return 'roadside assistance';
      ...
      const estimatedFare = Math.max(baseFare, baseFare + Math.ceil(extraKm * perKmRate));
      ...
      const recoveryRequest = {
        ...
        discoveryFilters: {
          pinkCaptainOnly: !!(data?.pinkCaptainOnly || data?.preferences?.pinkCaptainOnly),
          safety: {
      ...
      for (const d of nearbyDrivers) {
        this.webSocketService.sendToUser(String(d.id), {
          ...
          data: {
            ...
            estimatedFare: bookingDoc.fareDetails?.estimatedFare || bookingDoc.fare || 0
          }
        });
      }
  ...
  async handleAcceptRequest(ws, message) {
    ...
    try {
      ...
      const booking = await Booking.findById(bid).select('fareDetails user driver status');
      if (!booking) {
        throw new Error('Booking not found');
      }
      const negotiationState = booking.fareDetails?.negotiation?.state;
      const hasFinalFare = typeof booking.fareDetails?.finalFare === 'number' && booking.fareDetails.finalFare > 0;
      if (negotiationState !== 'accepted' && !hasFinalFare) {
        throw new Error('Price must be accepted before driver can accept the job');
      }
      ...
      const shouldFavorite = data?.favorite === true || recoveryRequest?.discoveryFilters?.pinkCaptainOnly === true;
      if (shouldFavorite && recoveryRequest.userId) {
        try {
          ...
          logger.warn('Failed to add driver to favorites:', favErr?.message || favErr);
        }
      }
  ...
  async handleCancelRequest(ws, message) {
    ...
    try {
      ...
      const requesterId = (data.userId || ws?.user?.id || ws?.user?._id)?.toString?.();
      const isAdmin = ws?.user?.role === 'admin' || ws?.user?.role === 'superadmin';
      const isCreator = recoveryRequest.userId && requesterId && recoveryRequest.userId.toString() === requesterId;
      const isAssignedDriver = recoveryRequest.driverId && requesterId && recoveryRequest.driverId.toString() === requesterId;
      ...
      recoveryRequest.statusHistory.push({
        ...
        message: `Request cancelled by ${isAdmin ? 'admin' : isAssignedDriver ? 'driver' : 'user'}`
      });
      ...
      if (recoveryRequest.driverId) {
        this.webSocketService.sendToUser(recoveryRequest.driverId, {
          ...
          data: {
            ...
            reason: recoveryRequest.cancellationReason,
            cancelledBy: isAssignedDriver ? 'driver' : isAdmin ? 'admin' : 'user'
          }
        });
      }
      ...
      try {
        ...
        if (userId) {
          const count = await Booking.countDocuments({ user: userId, status: 'cancelled', cancelledAt: { $gte: since } });
          if (count >= redThreshold) {
            this.emitToClient(ws, { event: 'cancellation.redFlag', requestId, data: { count, windowDays: 30 } });
          } else if (count >= threshold) {
            this.emitToClient(ws, { event: 'cancellation.warning', requestId, data: { count, windowDays: 30 } });
          }
        }
      } catch (warnErr) {
        logger.warn('Cancellation warning check failed:', warnErr?.message || warnErr);
      }
  ...
  async handleGetDrivers(ws, message) {
    ...
    try {
      ...
      const radiusKm = Math.max(1, Math.min(50, Number(data?.searchRadius) || recoveryRequest.searchRadiusKm || 10));
      const onlyAssignable = data?.onlyAssignable === undefined ? true : !!data?.onlyAssignable; // default to only online

      const availableDrivers = await this.getAvailableDrivers(
        recoveryRequest.pickupLocation,
        radiusKm,
        { ...(recoveryRequest.discoveryFilters || {}), onlyAssignable, multiStopEnabled: !!recoveryRequest?.multiStop?.enabled }
      );
  ...
  async handleServiceComplete(ws, message) {
    ...
    try {
      ...
      const surgeMultiplier = process.env.SURGE_LEVEL === '2.0x' ? 2.0 : (process.env.SURGE_LEVEL === '1.5x' ? 1.5 : 1.0);
      ...
      if (recoveryRequest?.routeType === 'two_way' || recoveryRequest?.roundTrip === true) {
      ...
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'SERVICE_COMPLETE_ERROR', message: error.message } });
    }
  }
  ...
  async handleRecoveryMessageSend(ws, message) {
    const { requestId, data } = message || {};
    try {
      const { message: text, type = 'text', location, language, voiceToText, attachments } = data || {};
      if (!text && type === 'text') throw new Error('message is required');
      if ((type === 'image' || type === 'video') && (!text || !/^https?:\/\//i.test(text))) {
      ...
      if (type === 'location') {
        ...
        if (typeof lat !== 'number' || typeof lng !== 'number') {
          throw new Error('Valid location coordinates are required for location messages');
        }
      }
      ...
      const senderType = booking.user?.toString() === String(senderId) ? 'user' : 'driver';
      const newMsg = {
        sender: senderId,
        senderType,
        message: text || '',
        timestamp: new Date(),
        messageType: ['image', 'video', 'location'].includes(type) ? type : 'text',
        location: type === 'location' ? { type: 'Point', coordinates: [location.longitude ?? location.lng, location.latitude ?? location.lat] } : undefined,
        language: language || undefined,
        voiceToText: voiceToText || undefined,
        attachments: Array.isArray(attachments) ? attachments.filter(a => a?.url).map(a => ({ url: a.url, type: a.type || 'file', mime: a.mime, sizeBytes: a.sizeBytes })) : undefined
      };
      ...
      this.emitToClient(ws, { event: 'recovery.message.sent', requestId, data: { bookingId: bid, message: newMsg } });
    } catch (error) {
      logger.error('Error in handleRecoveryMessageSend:', error);
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'CHAT_SEND_ERROR', message: error.message } });
    }
  }
  ...
  async handleRecoveryMessagesGet(ws, message) {
    ...
    try {
      const bid = data?.bookingId || requestId;
      const booking = await Booking.findById(bid).select('messages user driver');
      if (!booking) throw new Error('Booking not found');
      this.emitToClient(ws, { event: 'recovery.messages', requestId, data: { bookingId: bid, messages: booking.messages || [] } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'CHAT_GET_ERROR', message: error.message } });
    }
  }
  ...
  async handleRecoveryMessageRead(ws, message) {
    ...
    try {
      ...
      this.emitToClient(ws, { event: 'recovery.message.read.ack', requestId, data: { bookingId: bid } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'CHAT_READ_ERROR', message: error.message } });
    }
  }
  ...
  async handleRecoveryTyping(ws, message) {
    ...
    try {
      ...
      if (!booking) throw new Error('Booking not found');
      const senderId = ws?.user?.id || ws?.user?._id;
      const recipient = String(booking.user) === String(senderId) ? booking.driver : booking.user;
      if (recipient) {
        this.webSocketService.sendToUser(String(recipient), { event: 'recovery.typing', requestId, data: { bookingId: bid, typing: true } });
      }
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'CHAT_TYPING_ERROR', message: error.message } });
    }
  }

  /** Price negotiation handlers **/
  async handlePricePropose(ws, message) {
    const { requestId, data } = message || {};
    try {
      const { proposedAmount } = data || {};
      if (typeof proposedAmount !== 'number') throw new Error('proposedAmount is required');
      const bid = data?.bookingId || requestId;
      ...
      const base = booking.fareDetails?.estimatedFare || 0;
      ...
      if (proposedAmount < min || proposedAmount > max) {
        throw new Error(`Proposed amount out of bounds (${min} - ${max})`);
      }

      booking.fareDetails = booking.fareDetails || {};
      booking.fareDetails.negotiation = { state: 'proposed', proposedAmount, by: (ws?.user?.id || ws?.user?._id), at: new Date(), bounds: { min, max } };
      await booking.save();

      // Notify the counterparty
      const recipient = String(booking.user) === String(ws?.user?.id || ws?.user?._id) ? booking.driver : booking.user;
      if (recipient) {
        this.webSocketService.sendToUser(String(recipient), {
          event: 'price.proposed', requestId, data: { bookingId: bid, proposedAmount, bounds: { min, max } }
        });
      }

      this.emitToClient(ws, { event: 'price.propose.ack', requestId, data: { bookingId: bid, proposedAmount } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'PRICE_PROPOSE_ERROR', message: error.message } });
    }
  }

  async handlePriceAccept(ws, message) {
    const { requestId, data } = message || {};
    try {
      const bid = data?.bookingId || requestId;
      const booking = await Booking.findById(bid);
      if (!booking?.fareDetails?.negotiation) throw new Error('No pending negotiation');
      ...
      await booking.save();
      this.emitToClient(ws, { event: 'price.accepted', requestId, data: { bookingId: bid, amount: booking.fareDetails.finalFare } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'PRICE_ACCEPT_ERROR', message: error.message } });
    }
  }

  async handlePriceReject(ws, message) {
    const { requestId, data } = message || {};
    try {
      const bid = data?.bookingId || requestId;
      const booking = await Booking.findById(bid);
      ...
      await booking.save();
      this.emitToClient(ws, { event: 'price.rejected', requestId, data: { bookingId: bid } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'PRICE_REJECT_ERROR', message: error.message } });
    }
  }
  ...
  async handleDiscoveryFiltersSet(ws, message) {
    ...
    try {
      ...
      if ((data?.pinkCaptainOnly || data?.preferences?.pinkCaptainOnly) && (rec.serviceType?.toLowerCase?.() === 'towing' || rec.serviceType?.toLowerCase?.().includes('recovery') || rec.bookingId)) {
        // Our booking-backed flow marks serviceType as 'car recovery' in DB
        const booking = await Booking.findById(rec.bookingId).select('serviceType');
        const isCarRecovery = booking?.serviceType === 'car recovery';
        if (isCarRecovery) {
          throw new Error('Pink captains are only available for Bike and Cab services');
        }
      }
      rec.discoveryFilters = {
        pinkCaptainOnly: !!data?.pinkCaptainOnly,
        safety: {
          familyWithGuardianMale: !!data?.safety?.familyWithGuardianMale,
          noMaleCompanion: !!data?.safety?.noMaleCompanion,
          maleWithoutFemale: !!data?.safety?.maleWithoutFemale
        }
      };
      this.activeRecoveries.set(requestId, rec);
      this.emitToClient(ws, { event: 'discovery.filters.updated', requestId, data: rec.discoveryFilters });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'FILTERS_SET_ERROR', message: error.message } });
    }
  }
  ...
  async handleFavouritesAdd(ws, message) {
    ...
    try {
      ...
      await User.findByIdAndUpdate(userId, { $addToSet: { favoriteDrivers: data.driverId } });
      this.emitToClient(ws, { event: 'favourites.added', requestId, data: { driverId: data.driverId } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'FAV_ADD_ERROR', message: error.message } });
    }
  }
  ...
  async handleFavouritesList(ws, message) {
    ...
    try {
      const user = await User.findById(ws?.user?.id || ws?.user?._id).populate('favoriteDrivers', 'firstName lastName phoneNumber currentLocation');
      this.emitToClient(ws, { event: 'favourites.list', requestId, data: { drivers: user?.favoriteDrivers || [] } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'FAV_LIST_ERROR', message: error.message } });
    }
  }
  ...
  async handleFavouritesRemove(ws, message) {
    ...
    try {
      const userId = ws?.user?.id || ws?.user?._id;
      if (!data?.driverId) throw new Error('driverId is required');
      await User.findByIdAndUpdate(userId, { $pull: { favoriteDrivers: data.driverId } });
      this.emitToClient(ws, { event: 'favourites.removed', requestId, data: { driverId: data.driverId } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'FAV_REMOVE_ERROR', message: error.message } });
    }
  }
  ...
  async handleBookingToFavourite(ws, message) {
    ...
    try {
      ...
      this.emitToClient(ws, { event: 'booking.toFavourite.ack', requestId, data: { driverId } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'BOOK_TO_FAV_ERROR', message: error.message } });
    }
  }
  ...
  async handleMultiStopSet(ws, message) {
    ...
    try {
      const rec = this.activeRecoveries.get(requestId);
      if (!rec) throw new Error('Recovery request not found');
      rec.multiStop = { enabled: !!data?.enabled, stops: Array.isArray(data?.stops) ? data.stops : [] };
      this.activeRecoveries.set(requestId, rec);
      this.emitToClient(ws, { event: 'multiStop.updated', requestId, data: rec.multiStop });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'MULTISTOP_SET_ERROR', message: error.message } });
    }
  }
  ...
  async handleMultiStopRules(ws, message) {
    ...
    this.emitToClient(ws, { event: 'multiStop.rules.response', requestId, data: { allowed: true, driverOptOut: true } });
  }
  ...
  async handleWaitingConsent(ws, message) {
    ...
    try {
      ...
      this.activeRecoveries.set(requestId, rec);
      this.emitToClient(ws, { event: 'service.waiting.consent.ack', requestId, data: rec.waitingConsent });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'WAITING_CONSENT_ERROR', message: error.message } });
    }
  }
  ...
  async getAvailableDrivers(pickupLocation, maxDistanceKm = 100, filters = {}) {
    try {
      ...
      const lat = pickupLocation.lat ?? pickupLocation.latitude ?? pickupLocation.coordinates?.lat ?? pickupLocation.coordinates?.latitude;
      const lng = pickupLocation.lng ?? pickupLocation.longitude ?? pickupLocation.coordinates?.lng ?? pickupLocation.coordinates?.longitude;
      ...
      const query = {
        role: 'driver',
        driverStatus: filters.onlyAssignable ? 'online' : { $in: ['online', 'on_ride', 'busy'] },
        currentLocation: {
          $near: {
      ...
      const drivers = await User.find(query)
        .limit(10)
        .select('_id firstName lastName phoneNumber currentLocation driverStatus');
  ...
  _getRecoveryPricing({ serviceType = 'towing', vehicleType = 'car', distance = 0 }) {
    const typeKey = serviceType && typeof serviceType === 'string' ? serviceType : 'towing';
  ...
  async handlePresenceEnroute(ws, message) {
    ...
    try {
      ...
      if (!rec) throw new Error('Recovery request not found');
      rec.status = 'enroute';
      rec.statusHistory.push({ status: 'enroute', timestamp: new Date(), driverId: data?.driverId });
      this.activeRecoveries.set(requestId, rec);
      this.emitToClient(ws, { event: 'presence.enroute', requestId, data: { status: 'enroute', timestamp: new Date() } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'PRESENCE_ENROUTE_ERROR', message: error.message } });
    }
  }
  ...
  async handlePresenceStatus(ws, message) {
    ...
    try {
      ...
      rec.statusHistory.push({ status, timestamp: new Date(), note: data?.note, driverId: data?.driverId });
      this.activeRecoveries.set(requestId, rec);
      this.emitToClient(ws, { event: 'presence.status', requestId, data: { status, note: data?.note, timestamp: new Date() } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'PRESENCE_STATUS_ERROR', message: error.message } });
    }
  }
  ...
  async handleDriverCancel(ws, message) {
    ...
    try {
      ...
      rec.status = 'pending';
      rec.driverId = null;
      rec.statusHistory.push({ status: 'driver_cancelled', timestamp: new Date(), driverId: data.driverId, reason: data?.reason });
      this.activeRecoveries.set(requestId, rec);
      ...
      const nearbyDrivers = await this.getAvailableDrivers(
        ...
        { ...(rec.discoveryFilters || {}), multiStopEnabled: !!rec?.multiStop?.enabled }
      );
      for (const d of nearbyDrivers) {
        this.webSocketService.sendToUser(String(d.id), {
          event: 'newRecoveryRequest',
      ...
      this.emitToClient(ws, { event: 'driver.autoTransfer.initiated', requestId, data: { candidates: nearbyDrivers.length } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'DRIVER_CANCEL_ERROR', message: error.message } });
    }
  }

  async handleFraudCheck(ws, message) {
    const { requestId } = message || {};
    try {
      // Simple placeholder: emit check started; real logic would query Booking history
      this.emitToClient(ws, { event: 'fraud.check.started', requestId, data: { timestamp: new Date() } });
      // TODO: Implement actual detection using Booking/user pair and pickup/drop coordinates
      this.emitToClient(ws, { event: 'fraud.check.completed', requestId, data: { suspicious: false } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'FRAUD_CHECK_ERROR', message: error.message } });
    }
  }
  ...
  async handleSavedLocationAdd(ws, message) {
    ...
    try {
      ...
      if (!data?.name || !data?.coordinates) throw new Error('name and coordinates are required');
      const lat = data.coordinates.lat ?? data.coordinates.latitude;
      const lng = data.coordinates.lng ?? data.coordinates.longitude;
      await User.findByIdAndUpdate(userId, { $addToSet: { savedLocations: { name: data.name, address: data.address || '', coordinates: { lat, lng } } } });
      this.emitToClient(ws, { event: 'savedLocations.added', requestId, data: { name: data.name } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'SAVED_LOC_ADD_ERROR', message: error.message } });
    }
  }
  async handleSavedLocationList(ws, message) {
    const { requestId } = message || {};
    try {
      const user = await User.findById(ws?.user?.id || ws?.user?._id).select('savedLocations');
      this.emitToClient(ws, { event: 'savedLocations.list', requestId, data: { locations: user?.savedLocations || [] } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'SAVED_LOC_LIST_ERROR', message: error.message } });
    }
  }
  async handleSavedLocationRemove(ws, message) {
    ...
    try {
      ...
      await User.findByIdAndUpdate(userId, { $pull: { savedLocations: { name: data.name } } });
      this.emitToClient(ws, { event: 'savedLocations.removed', requestId, data: { name: data.name } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'SAVED_LOC_REMOVE_ERROR', message: error.message } });
    }
  }
  ...
  async handleRatingSubmit(ws, message) {
    ...
    try {
      const bid = requestId;
      const booking = await Booking.findById(bid).select('user driver rating');
      if (!booking) throw new Error('Booking not found');
      const fromUserId = String(ws?.user?.id || ws?.user?._id);
      const isCustomer = String(booking.user) === fromUserId;
      const field = isCustomer ? 'driverRating' : 'userRating';
      const update = { $set: { [`rating.${field}.stars`]: Number(data?.stars || 0), [`rating.${field}.comment`]: data?.comment || '', [`rating.${field}.ratedAt`]: new Date() } };
      await Booking.findByIdAndUpdate(bid, update);
      this.emitToClient(ws, { event: 'rating.submitted', requestId, data: { role: isCustomer ? 'customer' : 'driver' } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'RATING_SUBMIT_ERROR', message: error.message } });
    }
  }
  async handleTipAdd(ws, message) {
    ...
    try {
      ...
      await Booking.findByIdAndUpdate(requestId, { $set: { 'paymentDetails.tip': amount } });
      this.emitToClient(ws, { event: 'tip.added', requestId, data: { amount } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'TIP_ADD_ERROR', message: error.message } });
    }
  }
  async handleBillingGet(ws, message) {
    const { requestId } = message || {};
    try {
      const booking = await Booking.findById(requestId).select('receipt fareDetails paymentDetails completedAt');
      if (!booking) throw new Error('Booking not found');
      this.emitToClient(ws, { event: 'billing.details', requestId, data: { receipt: booking.receipt || null, fareDetails: booking.fareDetails || {}, paymentDetails: booking.paymentDetails || {}, completedAt: booking.completedAt || null } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'BILLING_GET_ERROR', message: error.message } });
    }
  }
  ...
  async handleChatUpload(ws, message) {
    ...
    try {
      ...
      this.emitToClient(ws, { event: 'chat.uploaded', requestId, data: { url: fileUrl, mime } });
    } catch (error) {
      this.emitToClient(ws, { event: 'error', requestId, error: { code: 'CHAT_UPLOAD_ERROR', message: error.message } });
    }
  }
}
